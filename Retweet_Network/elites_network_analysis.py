...
# Point these paths to the graphs generated by the generate_graphs.py script in the Collective Influence directory
RT_GRAPHS_DIR_2016 = '../data/ci_output/graphs/2016/'
RT_GRAPHS_DIR_2020 = '../data/ci_output/graphs/2020/'

PATH_TO_BIAS_2016 = {
    os.path.join(RT_GRAPHS_DIR_2016, 'center_2016_ci.gt'): 'center',
    os.path.join(RT_GRAPHS_DIR_2016, 'left_extreme_2016_ci.gt'): 'left extreme',
    os.path.join(RT_GRAPHS_DIR_2016, 'right_extreme_2016_ci.gt'): 'right extreme',
    os.path.join(RT_GRAPHS_DIR_2016, 'fake_2016_ci.gt'): 'fake',
    os.path.join(RT_GRAPHS_DIR_2016, 'left_leaning_2016_ci.gt'): 'left leaning',
    os.path.join(RT_GRAPHS_DIR_2016, 'right_leaning_2016_ci.gt'): 'right leaning',
    os.path.join(RT_GRAPHS_DIR_2016, 'left_2016_ci.gt'): 'left',
    os.path.join(RT_GRAPHS_DIR_2016, 'right_2016_ci.gt'): 'right'
}

PATH_TO_BIAS_2020 = {
    os.path.join(RT_GRAPHS_DIR_2020, 'center_2020_ci.gt'): 'center',
    os.path.join(RT_GRAPHS_DIR_2020, 'left_extreme_2020_ci.gt'): 'left extreme',
    os.path.join(RT_GRAPHS_DIR_2020, 'right_extreme_2020_ci.gt'): 'right extreme',
    os.path.join(RT_GRAPHS_DIR_2020, 'fake_2020_ci.gt'): 'fake',
    os.path.join(RT_GRAPHS_DIR_2020, 'left_leaning_2020_ci.gt'): 'left leaning',
    os.path.join(RT_GRAPHS_DIR_2020, 'right_leaning_2020_ci.gt'): 'right leaning',
    os.path.join(RT_GRAPHS_DIR_2020, 'left_2020_ci.gt'): 'left',
    os.path.join(RT_GRAPHS_DIR_2020, 'right_2020_ci.gt'): 'right'
}
...
def load_graphs_gt(path, year='2020', completeness='simple', x=-3):
    ...
    for file in fnames:
        ...
        if year == '2020':
            ...
            bias = PATH_TO_BIAS_2020[os.path.join(path, file)]
        elif year == '2016':
...
def influencer_network_anlaysis(year=2020):
    path = RT_GRAPHS_DIR_2016 if year == 2016 else RT_GRAPHS_DIR_2020

    top_n_influencers = 30
    biased_graphs = load_graphs_gt(path)
...
def influence_gain(graphs_2016, graphs_2020, N=100, plot=False):
    ...
    user_map = pickle.load(open(USER_MAP_2020, 'rb')) #2020
    ...
    if plot:
        for year in [2016, 2020]:
            ...
            user_bias_cis.to_csv('./igloo_{}.csv'.format(year))
...
def influence_gain_unmapped(graphs_2016, graphs_2020, N=100, plot=False):
    influencers_by_bias_2016 = top_influencers(None, graphs_2016)
    influencers_by_bias_2020 = top_influencers(None, graphs_2020)
...
def sankey_data(df, bias):
    ...
    data.to_csv('./sankey_data_{}.csv'.format(bias))
...
def all_network_stats(completeness='simple'):
...
def main():
    ...
    graphs_2020 = load_graphs_gt(RT_GRAPHS_DIR_2020, year='2020')
    graphs_2016 = load_graphs_gt(RT_GRAPHS_DIR_2016, year='2016')

    # get the change in influence, per bias for each user
    delta_df = influence_gain(graphs_2016, graphs_2020, N=25, plot=False)

    # additional analysis on network of only the top influencers
    #influencer_network_anlaysis(year=2016)
    #influencer_network_anlaysis(year=2020)

    # statistics on each of the bias networks, nodes, edges, etc.
    #all_network_stats()
